...
JWT authentication gives full access to an admin via the [`sign-in`](https://api.numeraire.tech/docs#tag/authentication/POST/v1/auth/sign-in) endpoint. It only requires `jwt.secret` to be set in the configuration file (and optionally `jwt.token_expiry`).

The admin password is not set in the configuration file. On first launch you set it through the [`sign-up`](https://api.numeraire.tech/docs#tag/authentication/POST/v1/auth/sign-up) endpoint, which stores it hashed in the database. The admin username is fixed as `admin`.
