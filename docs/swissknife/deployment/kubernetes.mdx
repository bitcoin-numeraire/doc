...
:::note
Kubernetes uses the separated images, not the all-in-one bundle:

- The `swissknife` chart deploys the API-only `bitcoinnumeraire/swissknife-server` image on port `3000`.
- The `dashboard` chart deploys the standalone `bitcoinnumeraire/swissknife-dashboard` Next.js image on port `8080`.
:::
